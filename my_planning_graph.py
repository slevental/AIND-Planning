...
class PlanningGraph():
        ...
        for action in self.all_actions:
            ...
            if action_node.prenodes.issubset(self.s_levels[level]):
                for parent in self.s_levels[level]:
                    if parent in action_node.prenodes:
    ...
    def add_literal_level(self, level):
        ...
        for action in self.a_levels[level - 1]:
            for effect in self.s_levels[level]:
                if effect in action.effnodes:
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for effect in node_a1.effnodes:
            if PgNode_s(effect.symbol, not effect.is_pos) in node_a2.effnodes:
                return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        '''
        for effect in node_a1.effnodes:
            if PgNode_s(effect.symbol, not effect.is_pos) in node_a2.prenodes:
                return True
        for effect in node_a2.effnodes:
            if PgNode_s(effect.symbol, not effect.is_pos) in node_a1.prenodes:
                return True
        return False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for effect1 in node_a1.parents:
            for effect2 in node_a2.parents:
                if effect1.is_mutex(effect2):
                    return True
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        '''
        for effect1 in node_s1.parents:
            if effect1 in node_s2.parents or not any([effect1.is_mutex(e) for e in node_s2.parents]):
                return False
        for effect2 in node_s2.parents:
            if not any([effect2.is_mutex(e) for e in node_s1.parents]):
                return False

        return len(node_s2.parents) + len(node_s1.parents) > 0
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            lvl = [pos for pos, level in enumerate(self.s_levels) if PgNode_s(goal, True) in level]
            if lvl:
                level_sum += lvl[0]
